            ...
            <td style="width: 20px;">
                ...
                @else
                    <input
                        ...
                        onchange="singleSelect{{ $randomId }}('{id}', '{name}', '{price}')"
                        type="radio"
                        value="{id}"
                        data-checked
                    >
    ...
    <script>
        ...
        function saveSelected{{ $randomId }}() {
            ...
            $('.selected-value-{{ $randomId }}').each(function () {
                ...
                listHtml += '<li>'
                listHtml += '<input type="hidden" name="{{ $name }}@if(!isset($single))[]@endif" value="' + id + '">' + name
        ...
        function addToSelect{{ $randomId }}(id, name, price) {
            ...
            if (isExists.length > 0) {
                ...
                $('#selected-{{ $randomId }}')
                    .append('<input class="selected-value-{{ $randomId }}" type="hidden" data-name="' + name + ' - ' + price + '" value="' + id + '">')
            }
        }
