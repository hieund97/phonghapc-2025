    ...
    <div class="col-6 form-group image-box">
        ...
        <div class="input-group justify-content-center">
            <div class="preview-image-wrapper ld-img-preview">
                <img
                        class="preview_image"
                        src="{{ old('thumbnail') ?: (!empty($currentCategory) ? get_image_url($currentCategory->thumbnail, '') : '/preview-icon.png') }}"
                >
            </div>
            <span>
    ...
    <div class="col-6 form-group image-box">
        ...
        <div class="input-group justify-content-center">
            <div class="preview-image-wrapper ld-img-preview">
                <img
                        class="preview_image"
                        src="{{ old('banner') ?: (!empty($currentCategory) ? get_image_url($currentCategory->banner, '') : '/preview-icon.png') }}"
                >
            </div>
            <span>
                <a data-result="image" data-action="select-image" class="btn_gallery btn btn-primary text-white">
...
<div class="form-group">
    <label for="status">{{ __('Highlight') }}</label>
    <select
            name="highlight"
            id="highlight"
            class="form-control select2bs4 @error('highlight') is-invalid @enderror"
            required
    >
        <option
                value="0"
                @if(old('highlight') == 0 || (!empty($currentCategory) && $currentCategory->highlight == 0)) selected @endif>
            Không
        </option>
        <option
                value="1"
                @if(old('status') == 1 || (!empty($currentCategory) && $currentCategory->highlight == 1)) selected @endif>
            Có
        </option>
    </select>
