...
class RvMedia
{
    ...
    public function handleUpload($fileUpload, $folder_id = 0, $path = '')
    {
        ...
        try {
            ...
            $record = $this->fileRepository->createOrUpdate($file);
            ...
            if (is_image($mime_type) && $mime_type !== 'image/svg+xml') {
                ...
                foreach ($arrResize as $key=>$size) {
                    $readable_size = explode('x', $size);
                    $recordResize = $this->thumbnailService
                        ...
                        ->save();
                    $mediaInfo = $this->uploadManager->fileDetails($recordResize);

                    try {
                        ImageOptimizer::optimize(str_replace(
                            ['https://cdn.chungauto.vn', 'http://cdn.chungauto.vn'],
                            Storage::disk('public')->path('uploads'),
                            $mediaInfo["url"]
                        ));
                    } catch (Exception $e) {}
                }
